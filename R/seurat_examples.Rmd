...
source("Cell_markers.ident.R")
```



```{r pressure, echo=FALSE, fig.height=9, fig.width=12}
markers.allset <- read.csv(file=file.path("/home/ronghui/project/dbmaker/proteinatlas.ihc.csv"))[ ,1]
...
# Idents(cbmc) <- "seurat_clusters"
markers.i <- identify_single_marker(cbmc, "6", markers.allset)
```
...
plot_combine_PRAUC(cbmc, "CD79B", "CD74", "6", return.obj = T) + ggtitle(label = "PRAUC")
plot_combine_PRAUC(cbmc, "FCRLA", "HLA-DRA", "6", return.obj = T) + ggtitle(label = "COMET combine markers")
...
for (x.cluster in seq(1,6)) {
  ...
  # genes <- c(df.split[1,1], genes[2,1])
