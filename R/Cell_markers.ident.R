...
get_gene_score_pos <- function(scrna, gene, id, step = 0.01, assay = "RNA", slot = "data") {
  scrna@active.assay <- assay
...
get_gene_score_neg <- function(scrna, gene, id, step = 0.01, assay = "RNA", slot = "data") {
  scrna@active.assay <- assay
...
Detect_single_marker <- function(scrna, id, step = 0.1,  slot = "data", category = NULL,
                          geneset = NULL, assay = "RNA", do.fast = F, min.pct = 0.1, use.all = F, do.f1score = F){
  if (is.null(geneset)) {
...
Detect_combine_markers <- function(scrna, id, step = 0.1, category = NULL,
                                   ...
                                   do.fast = F, min.pct = 0.1, use.all = F){
  if (is.null(geneset)) {
    geneset <- rownames(scrna[[assay]])
  }else{
    geneset <- intersect(rownames(scrna[[assay]]), geneset)
