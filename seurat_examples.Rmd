...
markers.i.5$gene
scrna <- cbmc
Detect_combine_markers <- function(scrna, id, geneset, step = 0.1){
  x.split <- identify_single_marker(scrna, id, markers.allset, step = 0.1)
  x.split <- subset(df.split, prauc > 0.2)
  x.split <- marker_stepbystep(cbmc, "5", depth = 2, geneset = markers.i.5$gene)
  ...
  for (gene.iter in 1:length(gene.combn[,1])) {
    ...
    if (x.split[x.split$gene == gene1, ]$direction == "+") {
      if (x.split[x.split$gene == gene2, ]$direction == "+") {
        gene.prauc <- get_PRAUC_matrix_combine_markers(scrna = scrna, gene1 = gene1, gene2 = gene2, id = id, step = 0.01)
        
      }else{
        gene.prauc <- get_PRAUC_matrix_combine_markers_neg_pos(scrna = scrna, gene1 = gene1, gene2 = gene2, id = id, step = 0.01)
      }
    }else{
      if (x.split[x.split$gene == gene2, ]$direction == "+") {
        gene.prauc <- get_PRAUC_matrix_combine_markers_neg_pos(scrna = scrna, gene1 = gene2, gene2 = gene1, id = id, step = 0.01)
      }else{
        gene.prauc <- get_PRAUC_matrix_combine_markers_neg(scrna = scrna, gene1 = gene1, gene2 = gene2, id = id, step = 0.01)
      }
    }
  ...
  x.df <- get_PRAUC_matrix_combine_markers_neg_pos(scrna = scrna, gene1 = "MYL3", gene2 = "TMSB4X", id = id, step = 0.01)

  ggplot() + geom_line(x.df, mapping = aes(x.rec, x.pre)) + xlim(c(0,1)) + ylim(c(0,1))
  
  x.df <- get_PRAUC_matrix_combine_markers_neg(scrna = scrna, gene1 = "TMSB4X", gene2 = "MT-CO2", id = id, step = 0.01)

  ggplot() + geom_line(x.df, mapping = aes(x.rec, x.pre)) + xlim(c(0,1)) + ylim(c(0,1))
  
  x.df <- get_PRAUC_matrix_combine_markers(scrna = scrna, gene1 = "HIST2H2AB", gene2 = "MYL3", id = id, step = 0.01)
...
t(combn(x.split$gene, 2))[1,]


grid_plot(cbmc, x.split, "5")

df <- data.frame(exp1 = scrna@assays$RNA@data["MYL3",],
                 exp2 = scrna@assays$RNA@data["FTH1",],
                 ID = as.character(makeid(scrna, id)@active.ident))
...
Idents(cbmc) <- "seurat_clusters"
Combine_marker_umap(cbmc, x.split, "5")

gene.combn.prauc[order(gene.combn.prauc$PRAUC, decreasing = T),]
  ...
  x.df <- get_PRAUC_matrix_combine_markers_neg_pos(scrna = scrna, gene1 = "MYL3", gene2 = "FTH1", id = id, step = 0.01)

  ggplot() + geom_line(x.df, mapping = aes(x.rec, x.pre)) + xlim(c(0,1)) + ylim(c(0,1))
  
  x.df <- get_PRAUC_matrix_combine_markers_neg_pos(scrna = scrna, gene1 = "TUSC3", gene2 = "TMSB4X", id = id, step = 0.01)
...
```{r}
print(plot_combine_PRAUC_neg(cbmc, "FTH1", "UBA52", "5", return.obj = T) + ggtitle(label = "PRAUC"))
# print(plot_combine_PRAUC_neg(cbmc, "RPS9", "RPS12", "5", return.obj = T) + ggtitle(label = "PRAUC"))
print(plot_combine_PRAUC_neg(cbmc, "RPLP2", "UBA52", "5", return.obj = T) + ggtitle(label = "PRAUC"))
print(plot_combine_PRAUC_neg(cbmc, "FTH1", "RPL8", "5", return.obj = T) + ggtitle(label = "PRAUC"))

df <- data.frame(exp1 = scrna@assays$RNA@data["PTTG2",],
                 exp2 = scrna@assays$RNA@data["TUSC3",],
                 ID = as.character(makeid(cbmc, "5")@active.ident))
...
```{r}
cbmc.x <- subset(cbmc, idents = "5")
markers.i.5 <- subset(markers.i.5, prauc > 0.2)
markers.i.5.n <- markers.i.5[markers.i.5$direction == "-", ]
markers.i.5.p <- markers.i.5[markers.i.5$direction == "+", ]

x.data.p <- cbmc.x@assays$RNA@data[markers.i.5.p$gene, ]
x.data.p <- as.data.frame(x.data.p)

x.data.n <- cbmc.x@assays$RNA@data[markers.i.5.n$gene, ]
x.data.n <- as.data.frame(x.data.n)

x.data.n$max <- apply(x.data.n, 1, max)
x.data.n <- x.data.n$max - x.data.n
x.data.n <- x.data.n[, -length(colnames(x.data.n))]

x.data <- rbind(x.data.n, x.data.p)

x.data <- scale(x.data)
# set.seed(4)
km.res <- kmeans(x.data, 10)

table(km.res$cluster)
km.res$cluster

dist(x.data,p=2, method = "minkowski")
dist(x.data[c("FTH1", "UBA52"),],p=2, method = "maximum")
dist(x.data[c("FTH1", "UBA52"),],p=2, method = "minkowski")

result <- dist(x.data, method = "euclidean")
result_hc <- hclust(d = result, method = "ward.D2")
#进行初步展示
fviz_dend(result_hc, cex = 0.6, k = 4)


x.data <- cbind(x.data, cluster = km.res$cluster)
